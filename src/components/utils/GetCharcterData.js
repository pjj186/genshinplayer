...
const GetCharcterData = ({ charcter, charImg, charBg, charName }) => {
  ...
  const getImgFile = async () => {
    const imgRef = ref(storageService, charImg);
  ...
  const getBackGround = async () => {
    const imgRef = ref(storageService, charBg);
    const url = await getDownloadURL(imgRef);
    const xhr = new XMLHttpRequest();
    xhr.responseType = "blob";
  ...
  const getMusicFile = async () => {
    ...
    const filename = list.items[charcter].name; // MP3 리스트 파일이름 가져오기
...
export default GetCharcterData;
